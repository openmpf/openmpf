...
public class MPFDetectionMessenger extends MPFMessengerBase {
    ...
    public MPFDetectionMessenger(MPFDetectionComponentBase detector, final String msgQueueName) throws JMSException {
		...
		try {
            ...
            if (msgMetadata != null) {
				...
				if(detector.supports(msgMetadata.getDataType())) {
					...
					MPFDetectionError returnError = null;
					...
					if (MPFDataType.AUDIO == msgMetadata.getDataType()) {
						...
						returnError = detector.getDetectionsFromAudio(
								msgMetadata.getJobName(),
								...
								tracks);
						responseBytes = detectionBuffer.createAudioResponseMessage(msgMetadata, detectionType, tracks, returnError);
						...
						returnError = detector.getDetectionsFromImage(
								msgMetadata.getJobName(),
								msgMetadata.getDataUri(),
								msgMetadata.getAlgorithmProperties(),
								msgMetadata.getMediaProperties(),
								locations);
						responseBytes = detectionBuffer.createImageResponseMessage(msgMetadata, detectionType, locations, returnError);
						...
						returnError = detector.getDetectionsFromVideo(
								msgMetadata.getJobName(),
								...
								msgMetadata.getMediaProperties(),
								tracks);
						responseBytes = detectionBuffer.createVideoResponseMessage(msgMetadata, detectionType, tracks, returnError);
					}


					// for debugging purposes
					if (MPFDetectionError.MPF_DETECTION_SUCCESS == returnError) {
