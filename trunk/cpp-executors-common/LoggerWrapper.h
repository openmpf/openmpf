...
private:
    std::shared_ptr<std::string> logger_ctx_ref_;
    std::string previous_ctx_msg_;
...
public:
    ...
    [[nodiscard]] LoggerContext GetLoggerContext(std::string_view context_message) {
        return {context_message, current_ctx_msg_, base_logger_};
    }
...
private:
    ...
    bool error_enabled_;
    bool fatal_enabled_;
    std::shared_ptr<std::string> current_ctx_msg_ = std::make_shared<std::string>();

    template<typename Arg, typename... Args>
    static auto ToString(Arg&& arg, Args&&... args) {
        if constexpr (sizeof...(Args) == 0 && std::is_convertible_v<Arg, std::string_view>) {
            // Don't bother with the stringstream when logging a single string.
