...
namespace MPF { namespace COMPONENT {
    ...
    StreamingVideoCapture::StreamingVideoCapture(const log4cxx::LoggerPtr &logger,
                                                 const MPFFrameReaderJob &job)
            : logger_(logger)
            , video_uri_(job.stream_uri)
            , job_(job)
        ...
        stream_properties["STREAM_POSITION_MSEC"] = cv_video_capture_.get(CAP_PROP_POS_MSEC);
        stream_properties["STREAM_FRAMES_PER_SEC"] = cv_video_capture_.get(CAP_PROP_FPS);
        stream_properties["FRAME_WIDTH"] = cv_video_capture_.get(CAP_PROP_FRAME_WIDTH);
        stream_properties["FRAME_HEIGHT"] = cv_video_capture_.get(CAP_PROP_FRAME_HEIGHT);
        stream_properties["FRAME_DATA_TYPE"] = cv_video_capture_.get(CAP_PROP_FORMAT);
    }
