...
import org.mitre.mpf.wfm.camel.TrackOutputHelper.TrackInfo;
...
public class JobCompleteProcessorImpl extends WfmProcessor implements JobCompleteProcessor {
    ...
    private void addTracksForMedia(
        ...
        var annotatorTracks = new HashMap<String, TrackInfo>();
        var actionWithAnnotators = new HashMap<Action, List<String>>();
        ...
        for (int taskIndex = (media.getCreationTask() + 1); taskIndex < job.getPipelineElements().getTaskCount(); taskIndex++) {
            ...
            for (int actionIndex = 0; actionIndex < task.actions().size(); actionIndex++) {
                ...
                var annotators = getAnnotators(job, media, taskIndex, action);
                ...
                if (!trackInfo.hadAnyTracks()) {
                    ...
                    // save off the annotator tracks
                    annotatorTracks.put(actionName, trackInfo);
                }
                else if (!annotators.isEmpty()) {
                    // save off the annotator lists for the action
                    actionWithAnnotators.put(action, annotators);
                }
                else {
                    trackCounter.add(media, trackInfo.tracksGroupedByAction().size());
                    addJsonTracks(
                            mediaOutputObject, job, media, action,
                            trackInfo.tracksGroupedByAction(), annotators);
                }
            }
        }

        addAnnotatedTrackInfo(job, media, mediaOutputObject, annotatorTracks, actionWithAnnotators, trackCounter);
        addMissingTrackInfo(noOutputActions, mediaOutputObject);
    }

    private void addAnnotatedTrackInfo(BatchJob job, Media media, JsonMediaOutputObject mediaOutputObject,
            HashMap<String, TrackInfo> annotatorTracks, HashMap<Action, List<String>> actionWithAnnotators,
            TrackCounter trackCounter) {
        // For each action with annotators, add the json tracks from those annotators
        for(Map.Entry<Action, List<String>> entry : actionWithAnnotators.entrySet()) {
            Action action = entry.getKey();
            List<String> annotators = entry.getValue();

            if (!annotators.isEmpty()) {
                // Get the last annotator in the list, which is the latest track generated by an annotator.
                String annotator = annotators.get(annotators.size() - 1);
                TrackInfo trackInfo = annotatorTracks.get(annotator);

                if(trackInfo != null) {
                    System.out.println(trackInfo.tracksGroupedByAction());
                    // Create the Multimap of tracks grouped by action name
                    Multimap<String, Track> actionTracks = HashMultimap.<String, Track>create();
                    for (Map.Entry<String, Track> trackEntry : trackInfo.tracksGroupedByAction().entries()) {
                        Track track = trackEntry.getValue();
                        actionTracks.put(action.name(), track);
                    }

                    // Update the counter and add the json track to the mediaOutputObject
                    trackCounter.add(media, actionTracks.size());
                    addJsonTracks(
                            mediaOutputObject, job, media, action,
                            actionTracks,
                            annotators);
    ...
    private List<String> getAnnotators(BatchJob job, Media media, int taskIndex, Action action) {
        List<String> annotators = new ArrayList<>();

        // fetch the algorithm and track type for the current action
        Algorithm algorithm = job.getPipelineElements().getAlgorithm(action.algorithm());
        String trackType = algorithm.trackType();

        // search subsequent tasks to determine if there are annotator actions for the FF tracks
        for (int futureTaskIndex = (taskIndex + 1); futureTaskIndex < job.getPipelineElements().getTaskCount(); futureTaskIndex++) {
            Task futureTask = job.getPipelineElements().getTask(futureTaskIndex);

            for (int futureActionIndex = 0; futureActionIndex < futureTask.actions().size(); futureActionIndex++) {
                String futureActionName = futureTask.actions().get(futureActionIndex);
                Action futureAction = job.getPipelineElements().getAction(futureActionName);

                // fetch the algorithm and track type for the future action
                ...
                System.out.println("Checking annotator for action: " + action.name() +
                        ", trackType: " + trackType + ", algorithm: " + algorithm.name() + ", futureAction: " + futureActionName +
                        ", futureTrackType: " + futureTrackType + ", futureAlgorithm: " + futureAlgorithm.name());
    ...
    private boolean isAnnotator(BatchJob job, Media media, Action action) {
        String annotatorValue = aggregateJobPropertiesUtil.getValue(
    ...
    private void addJsonTracks(
        ...
        for (var entry : tracksGroupedByMergedAction.asMap().entrySet()) {
            ...
            jsonAction.getAnnotators().addAll(annotators);
