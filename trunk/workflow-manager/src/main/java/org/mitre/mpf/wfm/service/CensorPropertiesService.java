...
public class CensorPropertiesService {
    ...
    public <T extends Map<String, String>> T copyAndCensorProperties(
            Map<String, String> source, T destination) {
        Set<String> censoredProperties = _propertiesUtil.getCensoredOutputProperties();
        for (Map.Entry<String, String> entry : source.entrySet()) {
            if (censoredProperties.contains(entry.getKey())) {
                LOG.info("Censoring the \"{}\" property value in JSON output.", entry.getKey());
                destination.put(entry.getKey(), CENSORED_PROP_REPLACEMENT);
            }
            else {
