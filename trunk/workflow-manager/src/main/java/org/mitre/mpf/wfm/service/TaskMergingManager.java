...
public class TaskMergingManager {
    ...
    // The Long is the job id. The UUID is the uuid in the bread crumb header. The String is the
    // name of the algorithm that generated the feed forward track.
    ...
    // initially been generated by different algorithms. When necessary, the returned
    // RequestTaskMergingContext will keep track of what algorithm was actually used to generate
    // the feed forward track included in the detection request message.
    public RequestTaskMergingContext getRequestContext(
        ...
        if (needsBreadCrumb(job, media, taskIdx, actionIdx)) {
            return (detectionRequestMessage, feedForwardTrack)
                    -> addBreadCrumb(detectionRequestMessage, feedForwardTrack, job.getId());
        }
        else {
            return (detectionRequestMessage, feedForwardTrack) -> detectionRequestMessage;
        }
    }


    // When we get a detection response message, we must determine the algorithm used to generate
    // the feed forward track included in the original detection request message and remove the
    // entry in _trackSourceAlgos, if one exists. It is important that the entry is removed, even
    // when there is an error, in order to prevent a memory leak.
    ...
    private Message addBreadCrumb(
        ...
        message.setHeader(BREAD_CRUMB_HEADER, CUSTOM_BREAD_CRUMB_PREFIX + trackId);
        return message;
    }


    private static Optional<UUID> parseBreadCrumb(Map<String, Object> headers) {
        return Optional.ofNullable(headers.get(BREAD_CRUMB_HEADER))
    ...
    public interface RequestTaskMergingContext {
        public Message addBreadCrumbIfNeeded(
                Message detectionRequestMessage, Track feedForwardTrack);
    }
